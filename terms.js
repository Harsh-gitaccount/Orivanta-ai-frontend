...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Track page view
    OrivantaUtils.trackPageView('/terms');
...
function initializeScrollAnimations() {
    ...
    const sections = document.querySelectorAll('.terms-section, .terms-intro');
...
function trackScrollDepth() {
    ...
    window.addEventListener('scroll', OrivantaUtils.throttle(() => {
        ...
        milestones.forEach(milestone => {
            if (scrollPercentage >= milestone && !tracked.has(milestone)) {
                ...
                OrivantaUtils.trackEvent('Engagement', 'Scroll Depth', `Terms of Service - ${milestone}%`, milestone);
            }
        });
    }, 250));
    ...
    window.addEventListener('beforeunload', () => {
        ...
        OrivantaUtils.trackEvent('Engagement', 'Time on Page', 'Terms of Service', timeSpent);
    });
}
...
document.addEventListener('click', (e) => {
    ...
    if (link && link.href) {
        ...
        if (linkHref.startsWith('mailto:')) {
            OrivantaUtils.trackEvent('Contact', 'Click', `Terms Page - Email Link`, linkHref);
        } else if (linkHref.includes('contact.html')) {
            OrivantaUtils.trackEvent('Contact', 'Click', `Terms Page - Contact Form`, linkHref);
        } else if (link.classList.contains('btn')) {
            OrivantaUtils.trackEvent('CTA', 'Click', `Terms Page - ${linkText}`, linkHref);
        }
    }
});
...
function storeTermsView() {
    const termsView = {
        viewed: true,
        timestamp: new Date().toISOString()
    };
    
    OrivantaUtils.setStorage('termsOfServiceViewed', termsView);
}
...
function addPrintButton() {
    ...
    printButton.innerHTML = '🖨️ Print Terms';
    ...
    printButton.addEventListener('click', () => {
        OrivantaUtils.trackEvent('Action', 'Print', 'Terms of Service');
        window.print();
    });
